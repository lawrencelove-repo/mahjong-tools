/**
 * Popup page bootstrap for Filipino Hand Evaluation.
 */
(function () {
  ...
  function mountWhenReady() {
    ...
    if (!window.FilipinoHandEvaluator || !window.FilipinoScore) {
      root.innerHTML = `<h2>Build Your Hand</h2><p class="hv-note">Loading scoring engine…</p>`;
      window.addEventListener("filipino-hand-evaluator-ready", mountWhenReady, {
        once: true,
      });
      return;
    }
    root.replaceChildren();
    api = window.FilipinoHandEvaluator.mount(root, () => settings);
    syncChrome();
  }
